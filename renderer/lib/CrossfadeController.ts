...
export class CrossfadeController {
  ...
  private startTimeUpdates(): void {
    ...
    this.timeUpdateInterval = setInterval(() => {
      ...
      if (activeVoice.audio && activeVoice.isPlaying) {
        ...
        if (this.crossfadeInProgress && this.crossfadeStartTime && this.crossfadePhase === 'active') {
          ...
          if (activeVoice.audio) {
            activeVoice.audio.volume = Math.max(0, Math.min(1, currentGain * (this.isMuted ? 0 : this.volume)));
          }
          if (inactiveVoice.audio && inactiveVoice.isPlaying) {
            inactiveVoice.audio.volume = Math.max(0, Math.min(1, nextGain * (this.isMuted ? 0 : this.volume)));
          }
        ...
        if (currentTime >= 0 && duration > 0 && !isNaN(currentTime) && !isNaN(duration) && isFinite(currentTime) && isFinite(duration)) {
  ...
  async loadTrack(track: CrossfadeTrack, options: CrossfadeOptions): Promise<void> {
    if (this.isDestroyed) {
      console.warn('Attempted to load track on destroyed CrossfadeController, ignoring');
      return;
    }
    ...
    try {
      ...
      audio.addEventListener('error', (e) => {
        const errorMsg = e.target?.error?.message || e.message || 'Unknown error';
  ...
  async scheduleCrossfade(nextTrack: CrossfadeTrack): Promise<void> {
    ...
    try {
      ...
      // Notify UI that crossfade is starting
  ...
  seek(time: number): void {
    ...
    if (activeVoice.audio) {
      activeVoice.audio.currentTime = Math.max(0, Math.min(time, activeVoice.audio.duration || 0));
    }
  }
